...
std::string Recurse(int l, int r)
{
	...
	if (x.g == 1)
	{
		// doar o grupă, nu afișăm 1(...) ci doar ...
...
void Solve()
{
	...
	for (i = 1; i < N; i++)
	{
		for (k = 1; 2 * k <= i + 1; k++)
		{
			...
			if (g > 1 && k * g >= 5)				// len 5 e min pt care merită să fold (aaaa -> 4(a) - tot 4 char, dar aaaaa -> 5(a) - 5 char)
			{
		...
		for (j = 0; j + i < N; j++)
		{
			...
			for (it = P[i + j].begin(); it != P[i + j].end(); it++)
			{
				...
				for (g = 1; g <= x.g; g++)
				{
					...
					if (cost <= sol)						// <= :)
					{
						// grupa cu care am optimizat aici
						Sol[j][j + i] = grupa(x.k, g);
						sol = cost;
					}
				}
			}

			if (sol == 10000)							// nu am grupe -> costul e nr de caractere
			{
				sol = A[j][j + i - 1] + 1;
				Sol[j][j + i] = grupa(1, 1);
			}

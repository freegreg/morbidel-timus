 ...
 *
 *  solutie: dinamica: x[i] -> suma minima pentru a ajunge la statia i. Caut binar cele mai din stanga statii 
 *  p1, p2, p3 la o distanta cel mult l1, l2, l3 si x[i] = min(x[pk] + lk), k = 1..3
 */
...
void readdata()
{
	...
	scanf("%ld %ld %ld %ld %ld %ld %ld %ld %ld", &l1, &l2, &l3, &c1, &c2, &c3, &n, &s1, &s2);
	if (s1 > s2) 
		i = s1, s1 = s2, s2 = i;
	for (i = 2; i <= n; i++)
		scanf("%ld", &d[i]);
...
long scoate_mare(long l, long r, long lim, long node)
{
	...
	do
	{
		if (l > r) return r;
		m = (l + r) / 2;
		if (dist(m, node) > lim) l = m + 1;
		else 
		if (dist(m, node) <= lim)
		{
			if (dist(m - 1, node) > lim || m == 1) return m;
			r = m - 1;
		}
	}
	while (1);

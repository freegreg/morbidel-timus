/*
 *  ACM Online
 *  Cube on a walk - Problem 1016
 *
 *  solutie: branch (retinem tabla cu minimul Œn fiecare pozitie a cubului)
 *  sloboz de problem†, mi-a fost sil† s-o scriu, sper s† iau AC din prima
 *  altfel o s-o iau de pe forum... (am luat Accepted... belea!)
 */
...
int lv, ml, sl, sc, fl, fc, mcubs[9][9], is[9][9];
typedef int cube[7];
struct table
{
	cube c;
...
{
	char ch[2];
	scanf("%s", ch); sc = ch[0] - 96; sl = ch[1] - '0';
	scanf("%s", ch); fc = ch[0] - 96; fl = ch[1] - '0';
	mcubs[sl][sc] = 1;
	for (ch[0] = 1; ch[0] <= 6; ch[0]++)
		scanf("%d", &a[sl][sc][0].c[ch[0]]);
}

/*
 *  ACM Online
 *  Subnumber - Problem 1165 (super marfa problema!)
 *
 *  solutie: verific numerele cu 1, 2, 3... cifre (cu multe cazuri...)
 */

#include <stdio.h>
#include <string.h>

#define input "subnum.in"
#define output "subnum.out"
...
/*  verific† dac† one e corect, adun† 1 si merge Œn dreapta pƒn† la cap†t  */
...
void solve()
{
	...
	/*  acum aplic†m formula magic† pentru id  */
	GetId();
	sub(sol, pz - 1);

	f = fopen(output, "w");
	for (i = sol[0]; i; fprintf(f, "%d", sol[i--]));
	fprintf(f, "\n");
	fclose(f);
}
